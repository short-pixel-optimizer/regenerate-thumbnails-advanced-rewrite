   ...
   public function spio_get_backup($fullpath, $id)
   {
      ...
      if (function_exists('wpSPIO') && method_exists(\wpSPIO(), 'filesystem')) {
         ...
         if (is_object($mediaItem)) {
            // In New SPIO, the backupModel takes care of backups, in older versions, the same functions can be found in the mediaImage. 
            if (method_exists($fs, 'getBackupModel')) {
               $backupModel = $fs->getBackupModel($id, 'media');
               if ($backupModel->hasBackup($mediaItem)) {
                  $backupFile = $backupModel->getBackupFile($mediaItem);
                  if (false !== $backupFile) {
